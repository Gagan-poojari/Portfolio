...
const navLinks = [
  { name: 'Skills',   href: '#skills',   glyph: '⬡', num: '01' },
  { name: 'Courses',  href: '#courses',  glyph: '◎', num: '02' },
  { name: 'Projects', href: '#projects', glyph: '◈', num: '03' },
  { name: 'Contact',  href: '#contact',  glyph: '◇', num: '04' },
];
...
const cmdItems = [
  ...navLinks.map(l => ({ label: l.name, desc: `Jump to ${l.name}`, href: l.href,            type: 'nav',    glyph: l.glyph, color: '#a3a3a3' })),
...
function CommandPalette({ open, onClose }) {
  ...
  const results = query.trim()
    ? cmdItems.filter(i => i.label.toLowerCase().includes(query.toLowerCase()) || i.desc.toLowerCase().includes(query.toLowerCase()))
    : cmdItems;
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[999] left-1/2 top-[16%] w-[92vw] max-w-[520px]"
            style={{
...
function NavLink({ href, name, glyph, isActive }) {
  ...
  return (
    <motion.div ref={ref} style={{ x: sx, y: sy }} onMouseMove={onMove} onMouseEnter={() => setHovered(true)} onMouseLeave={onLeave}>
      <Link
        href={href}
        className="relative flex items-center gap-1.5 text-xs tracking-widest uppercase transition-colors duration-200"
        style={{
...
function SocialBtn({ name, url, icon: Icon, accent, hoverBg }) {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.95 }}
      className="relative flex items-center justify-center w-8 h-8 rounded-lg border transition-all duration-200 overflow-visible"
      style={{
...
const NavBar = () => {
  ...
  const [showCmdPulse,   setShowCmdPulse]   = useState(false);
  ...
  // Show cmd pulse hint after 3s
  useEffect(() => {
    const t = setTimeout(() => setShowCmdPulse(true), 3000);
  ...
  return (
    <>
      ...
      >
        {/* Scanline overlay when scrolled */}
        {scrolled && (
        ...
        <nav className="hidden md:flex items-center gap-7 mr-5">
          {navLinks.map((l, i) => (
            ...
            >
              <NavLink {...l} isActive={activeSection === l.href.replace('#', '')} />
            </motion.div>
          ))}
        </nav>
            ...
            <AnimatePresence>
              {showCmdPulse && (
                <motion.span
                  ...
                  style={{ boxShadow: '0 0 6px rgba(255,255,255,0.8)', animation: 'pulse 2s infinite' }}
                />
              )}
            </AnimatePresence>
          ...
          >
            ...
            <div className="relative px-6 py-5">
              ...
              <div className="flex flex-col">
                {navLinks.map((l, i) => {
                  const isAct = activeSection === l.href.replace('#', '');
                  return (
                    <motion.div
                      key={l.name}
                      initial={{ opacity: 0, x: -10 }}
                    ...
                    >
                      <Link
                        ...
                        className="flex items-center justify-between py-3.5 border-b group transition-all duration-200 cursor-none"
